...
- A kernel specifies the statements that are executed by each individual thread in the grid
- A memory space is a simplified view of how a processor accesses its memory in modern computers. A memory space is usually associated with each running application. The data to be processed by an application and instructions executed for the application are stored in locations in its memory space.
